...
impl MapGen for Circle {
    fn generate(&self, hex_map: &mut HexMap) {
        ...
        for hex in &mut hex_map.field {
            ...
            let dst_to_edge = hex_map.absolute_size_y / 2.0
                - (hex.center_y - hex_map.absolute_size_y / 2.0).abs()
                + random::<f32>();
            let rel_dst_to_edge = dst_to_edge / (hex_map.absolute_size_y / 2.0);

            // circular land
            if (dst_to_center_x + dst_to_center_y).sqrt()
                < (self.ring_size
                    + p.get([
                        hex.center_x as f64 * 0.1 + seed as f64,
                        hex.center_y as f64 * 0.1,
                    ]) as f32
                        * 5.0)
            {
