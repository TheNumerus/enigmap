...
impl Basic {
    fn get_triangle_pixels(
        ...
        for y in (upper.1)..=(lower.1) {
            ...
            if upper.0 > pointy.0 {
                let start = (((half_height + 1) - current_pos) as f32 / half_height as f32 * width)
                    as u32
                    + pointy.0;
    ...
    fn render_hex(&self, img: &mut ImageBuffer<Rgb<u8>, Vec<u8>>, hex: &Hex) {
        ...
        let center_point = (
            (hex.center_x * self.multiplier) as u32,
            (hex.center_y * self.multiplier) as u32,
        );
        ...
        for i in 0..6 {
            ...
            points.push((
                (coords.0 * self.multiplier) as u32,
                (coords.1 * self.multiplier) as u32,
            ));
        }
        // get hex pixels
