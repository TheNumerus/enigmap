...
impl Renderer for OGL {
    fn render(&self, map: &HexMap) -> RgbImage {
            ...
            let data = map
                ...
                .map(|hex| {
                    ...
                    if self.wrap_map {
                        vec.push(Attr {
                            world_position: (
                                hex.center_x - 0.5 - map.size_x as f32,
                                hex.center_y - RATIO / 2.0,
                            ),
                            color,
                        });
                        vec.push(Attr {
        ...
        let mut imgbuf: ImageBuffer<Rgb<u8>, Vec<u8>> =
            ImageBuffer::from_fn(target_size_x, target_size_y, |x, y| {
                ...
                Rgb([
                    tiles[tile_idx][index],
                    tiles[tile_idx][index + 1],
                    tiles[tile_idx][index + 2],
                ])
            });
